...
#' @param nfc_intake Cumulative intake of non-fiber carbohydrates (\% DM).
...
starter_met_energy <- function(cs_ndf = 12,
  ...
  lnCSNFCI <- ifelse(nfc_intake <= 15.8, log(nfc_intake), log(15.8))
  ...
  starter_dig_energy <- ifelse(starter_dig_energy <= 0 | nfc_intake == 0, 0, starter_dig_energy)

  starter_met_energy <- (1.01 * starter_dig_energy - 0.45) + (0.0046 * (cs_ee - 3))

  starter_met_energy <- ifelse(starter_met_energy <= 0 | nfc_intake == 0, 0, starter_met_energy)
